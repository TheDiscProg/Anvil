...
import io.github.thediscprog.anvil.exceptions.AnvilException.NoReaderFoundError
    ...
    readerOrBoolean match
      ...
      case _: Boolean       => {
        jdbcType match
          ...
          case _                             =>
            AnvilMonitor.counter(AnvilMetrics.jdbcReaderNotFound)
            throw NoReaderFoundError(
              AnvilMessage(AnvilMessageKey.NoReaderFoundError, Seq(jdbcType))
            )
      }
  }
}
